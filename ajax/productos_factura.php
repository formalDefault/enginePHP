<?php
	...
	/* Connect To Database*/
	require_once ("../funciones.php");//Contiene las variables de configuracion para conectar a la base de datos
	
	$action = (isset($_REQUEST['action'])&& $_REQUEST['action'] !=NULL)?$_REQUEST['action']:'';
	if($action == 'ajax'){
		// escaping, additionally removing everything that could be (html/javascript-) code
         $q = mysqli_real_escape_string($con2,(strip_tags($_REQUEST['q'], ENT_QUOTES)));
		 $aColumns = array('clave_sat', 'concepto');//Columnas de busqueda
		 $sTable = "conceptos";
		 $sWhere = "";
		if ( $_GET['q'] != "" )
		{
